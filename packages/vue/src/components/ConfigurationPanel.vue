  ...
  <div class="configuration-panel">
    <div v-for="config in configurations" :key="config.id" class="config-item">
      ...
      <select 
        ...
        @change="e => updateConfig(config.id, (e.target as HTMLSelectElement).value)"
      >
      ...
      <input 
        ...
        :id="config.id"
        :value="values[config.id]" 
        @input="e => updateConfig(config.id, (e.target as HTMLInputElement).value)"
      />
      ...
      <input 
        ...
        :max="config.max"
        :step="config.step"
        @input="e => updateConfig(config.id, Number((e.target as HTMLInputElement).value))"
      />
      ...
      <input 
        ...
        @change="e => updateConfig(config.id, (e.target as HTMLInputElement).checked)"
      />
...
import { Pooder, ContributionPointIds, ConfigurationContribution, ConfigurationService } from '@pooder/core';
...
const refreshConfigs = () => {
  ...
  const contribs = pooder.getContributions<ConfigurationContribution>(ContributionPointIds.CONFIGURATIONS);
  configurations.value = contribs.map(c => c.data);

  // Get current values
  configService = pooder.getService<ConfigurationService>('ConfigurationService');
  if (configService) {
