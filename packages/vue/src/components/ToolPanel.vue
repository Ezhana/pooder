      ...
      <div v-if="ext.options">
        <div v-for="(val, key) in ext.options" :key="key" class="config-item">
          ...
          <div v-if="getInputType(ext, String(key), val) === 'select'">
            <select :value="val" @change="(e) => updateOption(ext, key, (e.target as HTMLSelectElement).value)">
                <option v-for="opt in getSelectOptions(ext, String(key))" :key="opt.value" :value="opt.value">
                    {{ opt.label }}
                </option>
            </select>
          </div>
          ...
          <div v-else-if="getInputType(ext, String(key), val) === 'number'">
            <input 
                type="number" 
                :value="val" 
                @input="(e) => updateOption(ext, key, Number((e.target as HTMLInputElement).value))" 
            />
            ...
            <div class="switch-wrapper">
              <label class="switch">
                <input 
                    type="checkbox" 
                    :checked="Boolean(val)" 
                    @change="(e) => updateOption(ext, key, (e.target as HTMLInputElement).checked)" 
                />
          ...
          <div v-else>
            <input 
                type="text" 
                :value="val" 
                @input="(e) => updateOption(ext, key, (e.target as HTMLInputElement).value)" 
            />
          </div>
        </div>
      </div>
...
<script lang="ts">
import { defineComponent, ref, watch, PropType, reactive, isReactive } from 'vue';
  ...
  setup(props) {
    ...
    const updateExtensions = () => {
      if (props.editor) {
        ...
        extensionList.value = Array.from(props.editor.getExtensions().map(ext => {
    ...
    const getSchema = (ext: Extension, key: string): OptionSchema | undefined => {
    ...
    const getInputType = (ext: Extension, key: string, value: any): string => {
      ...
      if (typeof value === 'string') {
        ...
        if (k.includes('color') || k.includes('fill') || k.includes('stroke')) return 'color';
    ...
    const getSelectOptions = (ext: Extension, key: string): { label: string, value: any }[] => {
        const schema = getSchema(ext, key);
